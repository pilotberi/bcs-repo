	...
	for (var row = 0; row < sheetData.length; row++) {
		...
		if (rowData[1] && rowData[1].length > 0) {
			...
			if (checkStatus == true) {
				...
				data["output_signal"] = rowData[1] || null;
...
function validate(rowData) {
	...
	if (rowData[1] == null || rowData[1] === '') { //this is to validate output_signal
		s_info(Strings.VALIDATION_ERROR_TITLE, Strings.OUTPUTSIGNAL_INVALID_OUTPUT_SIGNAL, dlgOptions);
		return false;
	}
