/*
	...
	Version : 2024
	Generated On : Mon Mar 16 2026 12:06:25 GMT+0530 (India Standard Time)
	Description : Combine Master Ui for outputSignal
...
function onApplicationReady() {
  ...
  setMenu(document.getElementById('sideMenu'), 'bcssteam_outputSignal');
...
function initSheet() {
  ...
  editorSheet = jspreadsheet(sheetEl, {
    ...
    columns: [{
      ...
      {
        type: 'text',
        title: Strings.COL_LABEL_OUTPUT_SIGNAL,
        width: 300,
        align: 'left',
        wordWrap: true,
        readOnly: false
...
function registerEventListeners() {
  saveOutputSignalBtn = document.getElementById('saveOutputSignalBtn');
  saveOutputSignalBtn.addEventListener('click', onSaveOutputSignalBtnClick);
}
...
function onUser(resp) {
  ...
  clCommon.initLanguage(userObj.realm_lang, function() {
    ...
    if (typeof OutputSignalInterface === 'undefined')
      OutputSignalInterface = null;

    if (OutputSignalInterface && OutputSignalInterface.onLoad)
      OutputSignalInterface.onLoad();
    initControls();
...
function loadOutputSignalData() {
  clearSheet();
  clCommon.showLoader("listProgress");

  var beforeLoad = true;
  if (OutputSignalInterface && OutputSignalInterface.onBeforeLoad)
    beforeLoad = OutputSignalInterface.onBeforeLoad();
  if (beforeLoad == false) {
    return;
  }

  Service.getOutputSignal(function(resultObj) {
    ...
    if (resultObj.code == 200) { //On success response
      ...
      var dataArr = resultObj.data;
      if (OutputSignalInterface && OutputSignalInterface.onAfterLoad) {
        ...
        dataArr = OutputSignalInterface.onAfterLoad(isSuccessful, resultObj.data);
      }
      ...
      s_error(Strings.ERROR_TITLE, Strings.ERROR_LOAD_OUTPUTSIGNAL, dlgOptions);
    }
  });
...
function onLoadSuccessResponse(dataArr) {
  if (dataArr.length > 0) {
    for (var i = 0; i < dataArr.length; i++) {
      ...
      editorSheet.setValue('B' + rowId, dataObj.output_signal, true);
...
function onSaveOutputSignalBtnClick(e) {
  var masterObj = {
    "master_data": [],
  ...
  for (var row = 0; row < sheetData.length; row++) {
    ...
    if (rowData[1] && rowData[1].length > 0) {
      ...
      outputSignalArr.push(data);
    }
  }

  var checkStatus = validate(outputSignalArr);
  ...
  var finalData = [];
  outputSignalArr.forEach(function(item) {
  ...
  var outputSignalObj = true;
  if (OutputSignalInterface && OutputSignalInterface.onBeforeSave)
    outputSignalObj = OutputSignalInterface.onBeforeSave(masterObj);
  if (outputSignalObj == false) {
    return;
  }
  ...
  Service.saveOutputSignal(masterObj, function(resultObj) {
    clCommon.hideLoader("listProgress");
    if (resultObj.code == 200) {
      var isSuccessful = true;

      var outputSignalObj = true;
      if (OutputSignalInterface && OutputSignalInterface.onAfterSave)
        outputSignalObj = OutputSignalInterface.onAfterSave(isSuccessful, resultObj.data);
      ...
      s_error(Strings.OUTPUTSIGNAL_ERROR_TITLE, Strings.SAVE_OUTPUTSIGNAL_ERROR, dlgOptions);
    }
  });
}
...
function validate(dataArr) {
  ...
  dataArr.forEach(function(outputSignalObj) {
    var errorStr = "";
    var rowNum = outputSignalObj._rowNum;

    function addError(error) {
      errorStr += error + "<br>";
    }

    var rowTitle = outputSignalObj.name || outputSignalObj.undefined || outputSignalObj.id || "-";

    if (OutputSignalInterface && OutputSignalInterface.getRowTitle) {
      var customTitle = OutputSignalInterface.getRowTitle(outputSignalObj, dataSources);
    ...
    if (outputSignalObj.output_signal == null || outputSignalObj.output_signal === '') {
      addError(Strings.OUTPUTSIGNAL_INVALID_OUTPUT_SIGNAL);
    }

    // C. External Interface Validation
    if (OutputSignalInterface && OutputSignalInterface.addValidation) {
      var extErr = OutputSignalInterface.addValidation(outputSignalObj, dataSources);
  ...
  if (validateError.length > 0) {
    ...
    var promptTitle = (typeof Strings !== 'undefined' && Strings.VALIDATE_OUTPUTSIGNAL_PROMPT) ? Strings.VALIDATE_OUTPUTSIGNAL_PROMPT : "Validation Errors";
    var objTitle = (typeof Strings !== 'undefined' && Strings.OUTPUTSIGNAL_TITLE) ? Strings.OUTPUTSIGNAL_TITLE : "OutputSignal";
...
function onSheetChangedListener(instance, cell, col, row, value) {
  ...
  var returnData = true;
  if (OutputSignalInterface && OutputSignalInterface.onSheetChanged)
    returnData = OutputSignalInterface.onSheetChanged(instance, cell, col, row, value);
  if (returnData == false) {
    return;
  }
