/*
	...
	Version : 2024
	Generated On : Mon Mar 16 2026 12:28:03 GMT+0530 (India Standard Time)
	Description : Combine Master Ui for workingPrinciple
...
function onApplicationReady() {
  ...
  setMenu(document.getElementById('sideMenu'), 'bcssteam_workingPrinciple');
...
function initSheet() {
  ...
  editorSheet = jspreadsheet(sheetEl, {
    ...
    columns: [{
      ...
      {
        type: 'text',
        title: Strings.COL_LABEL_WORKING_PRINCIPLE,
        width: 300,
        align: 'left',
        wordWrap: true,
        readOnly: false
...
function registerEventListeners() {
  saveWorkingPrincipleBtn = document.getElementById('saveWorkingPrincipleBtn');
  saveWorkingPrincipleBtn.addEventListener('click', onSaveWorkingPrincipleBtnClick);
}
...
function onUser(resp) {
  ...
  clCommon.initLanguage(userObj.realm_lang, function() {
    ...
    if (typeof WorkingPrincipleInterface === 'undefined')
      WorkingPrincipleInterface = null;

    if (WorkingPrincipleInterface && WorkingPrincipleInterface.onLoad)
      WorkingPrincipleInterface.onLoad();
    initControls();
...
function loadWorkingPrincipleData() {
  clearSheet();
  clCommon.showLoader("listProgress");

  var beforeLoad = true;
  if (WorkingPrincipleInterface && WorkingPrincipleInterface.onBeforeLoad)
    beforeLoad = WorkingPrincipleInterface.onBeforeLoad();
  if (beforeLoad == false) {
    return;
  }

  Service.getWorkingPrinciple(function(resultObj) {
    ...
    if (resultObj.code == 200) { //On success response
      ...
      var dataArr = resultObj.data;
      if (WorkingPrincipleInterface && WorkingPrincipleInterface.onAfterLoad) {
        ...
        dataArr = WorkingPrincipleInterface.onAfterLoad(isSuccessful, resultObj.data);
      }
      ...
      s_error(Strings.ERROR_TITLE, Strings.ERROR_LOAD_WORKINGPRINCIPLE, dlgOptions);
    }
  });
...
function onLoadSuccessResponse(dataArr) {
  if (dataArr.length > 0) {
    for (var i = 0; i < dataArr.length; i++) {
      ...
      editorSheet.setValue('B' + rowId, dataObj.working_principle, true);
...
function onSaveWorkingPrincipleBtnClick(e) {
  var masterObj = {
    "master_data": [],
  ...
  for (var row = 0; row < sheetData.length; row++) {
    ...
    if (rowData[1] && rowData[1].length > 0) {
      ...
      // Assign values
      data.id = rowData[0];
      data.working_principle = rowData[1];
      ...
      workingPrincipleArr.push(data);
    }
  }

  var checkStatus = validate(workingPrincipleArr);
  ...
  var finalData = [];
  workingPrincipleArr.forEach(function(item) {
  ...
  var workingPrincipleObj = true;
  if (WorkingPrincipleInterface && WorkingPrincipleInterface.onBeforeSave)
    workingPrincipleObj = WorkingPrincipleInterface.onBeforeSave(masterObj);
  if (workingPrincipleObj == false) {
    return;
  }
  ...
  Service.saveWorkingPrinciple(masterObj, function(resultObj) {
    clCommon.hideLoader("listProgress");
    if (resultObj.code == 200) {
      var isSuccessful = true;

      var workingPrincipleObj = true;
      if (WorkingPrincipleInterface && WorkingPrincipleInterface.onAfterSave)
        workingPrincipleObj = WorkingPrincipleInterface.onAfterSave(isSuccessful, resultObj.data);
      ...
      s_error(Strings.WORKINGPRINCIPLE_ERROR_TITLE, Strings.SAVE_WORKINGPRINCIPLE_ERROR, dlgOptions);
    }
  });
}
...
function validate(dataArr) {
  ...
  dataArr.forEach(function(workingPrincipleObj) {
    var errorStr = "";
    var rowNum = workingPrincipleObj._rowNum;

    function addError(error) {
      errorStr += error + "<br>";
    }

    var rowTitle = workingPrincipleObj.name || workingPrincipleObj.undefined || workingPrincipleObj.id || "-";

    if (WorkingPrincipleInterface && WorkingPrincipleInterface.getRowTitle) {
      var customTitle = WorkingPrincipleInterface.getRowTitle(workingPrincipleObj, dataSources);
    ...
    if (workingPrincipleObj.working_principle == null || workingPrincipleObj.working_principle === '') {
      addError(Strings.WORKINGPRINCIPLE_INVALID_WORKING_PRINCIPLE);
    }

    // C. External Interface Validation
    if (WorkingPrincipleInterface && WorkingPrincipleInterface.addValidation) {
      var extErr = WorkingPrincipleInterface.addValidation(workingPrincipleObj, dataSources);
  ...
  if (validateError.length > 0) {
    ...
    var promptTitle = (typeof Strings !== 'undefined' && Strings.VALIDATE_WORKINGPRINCIPLE_PROMPT) ? Strings.VALIDATE_WORKINGPRINCIPLE_PROMPT : "Validation Errors";
    var objTitle = (typeof Strings !== 'undefined' && Strings.WORKINGPRINCIPLE_TITLE) ? Strings.WORKINGPRINCIPLE_TITLE : "WorkingPrinciple";
...
function onSheetChangedListener(instance, cell, col, row, value) {
  ...
  var returnData = true;
  if (WorkingPrincipleInterface && WorkingPrincipleInterface.onSheetChanged)
    returnData = WorkingPrincipleInterface.onSheetChanged(instance, cell, col, row, value);
  if (returnData == false) {
    return;
  }
