/*
	...
	Version : 2024
	Generated On : Thu Jan 22 2026 21:03:04 GMT+0530 (India Standard Time)
...
var utilitySfm_code_id, utilityValue, utilityReason, utilityRemarks;
var sfmArr;
...
function initControls() {
  ...
  utilitySfm_code_id = document.getElementById('utilitySfm_code_id');
...
function registerEventListeners() {
  ...
  sfmCodeFilter = document.getElementById('sfmCodeFilter');
  sfmCodeFilter.addEventListener('change', onChangeOfFilter);
  ...
  utilitySfm_code_id.addEventListener('change', onFieldChange);
  utilitySfm_code_id.addEventListener('focus', onFieldFocus);
  utilitySfm_code_id.addEventListener('blur', onFieldChange);

  utilitySfm_code_idZoom = document.getElementById('utilitySfm_code_idZoom');
  utilitySfm_code_idZoom.addEventListener('click', onToolBarBtnClick);
    ...
    return function(respObj) {
      ...
      if (respObj.code == 200) {
        ...
        for (var t = 0; t < dataSources.getAllSfmWithoutLimit.length; t++) {
          ...
          sfmCodeFilter.innerHTML += '<option value="' + dataSources.getAllSfmWithoutLimit[t]._sid + '">' + dataSources.getAllSfmWithoutLimit[t].sfm_code + '</option>';
        }
...
function loadUtilityData() {
  ...
  var params = [sfmCodeFilter.value, dataSources.startDate, dataSources.endDate, offset, limit, ];
...
function onChangeOfFilter(e) {
  ...
  clCommon.setCookie("sfm_id", sfmCodeFilter.value, 10)
...
function onCreateUtilityBtnClick(e) {
  ...
  utilityObj.sfm_code_id = utilitySfm_code_id.getAttribute('Sfm_code_id');
...
function onRowClick(e) {
  ...
  if (!selectedRow) { //executes when the row is clicked for the first time
    ...
    clCommon.focus(utilitySfm_code_id);
    selectedRow = this;

    rowData = utilityTable.row(selectedRow).data();


    utilitySfm_code_id.setAttribute("Sfm_code_id", rowData.sfm_code_id ? rowData.sfm_code_id : "");
    utilitySfm_code_id.dispatchEvent(new Event('change'));

    selectedSfm_code_idId = rowData.sfm_code_id ? rowData.sfm_code_id : "";

    var sfmArr = dataSources.getAllSfmWithoutLimit; // Dynamically fetch array
    for (var i = 0; i < sfmArr.length; i++) {
      var obj = sfmArr[i];
      if (rowData.sfm_code_id == obj._sid) {
        utilitySfm_code_id.value = obj.sfm_code ? obj.sfm_code : "";
        break;
      }
    }
    ...
    var reasonCodeArr = dataSources.getSfmReadingReasonCode; // Dynamically fetch array
...
function displayForm(rowData) {
  ...
  utilitySfm_code_id.value = rowData.sfm_code_id;
  utilitySfm_code_id.dispatchEvent(new Event("change"));
...
function clearForm() {
  ...
  utilitySfm_code_id.setAttribute("Sfm_code_id", "");
...
function onDeleteUtilityBtnClick() {
  ...
  if (clArr.contains('disabled') == false)
    s_confirm(Strings.CONFIRM, Strings.UTILITY_DELETE_CONFIRM + " - " + rowData.sfm_code_name + Strings.DELETE_TITLE_UTILITY + " ?", { //Confirmation with user
      STR_YES: dlgOptions.STR_YES,
      STR_NO: dlgOptions.STR_NO,
...
function onFieldChange(e) {
  ...
  if (changedField.id == "utilitySfm_code_id") {
    for (var dt in dataSources.getAllSfmWithoutLimit) {
      var dtObj = dataSources.getAllSfmWithoutLimit[dt];
      if (selectedSfm_code_idId && selectedSfm_code_idId == dtObj._sid) {
        utilitySfm_code_id.value = dtObj.sfm_code ? dtObj.sfm_code : "";
        break;
      }
    }
  }
  ...
  if (changedField.id == "utilityReason") {
    for (var dt in dataSources.getSfmReadingReasonCode) {
      var dtObj = dataSources.getSfmReadingReasonCode[dt];
      if (selectedReasonId && selectedReasonId == dtObj.id) {
        utilityReason.value = dtObj.reading_reason_code ? dtObj.reading_reason_code : "";
        break;
      }
    }
  }
...
function validate(utilityObj) {
  ...
  if (!utilityObj.sfm_code_id) {
    s_info(Strings.VALIDATION_ERROR_TITLE, Strings.UTILITY_INVALID_SFM_CODE_ID, dlgOptions);
    return false;
  }
...
function onSfm_code_idZoomBtn(e) {
  ...
  var sfm_code_idArr = [];
  var sfm_code_idFlg = false;
  var objective = {
    "_sid": utilitySfm_code_id.getAttribute("Sfm_code_id")
  };
  var check = true;
  if (UtilityInterface && UtilityInterface.onLookUpData) {
    check = UtilityInterface.onLookUpData("sfm_code_id", dataSources);
  }
  ...
  for (var item of dataSources.getAllSfmWithoutLimit) {
    ...
    if (objective._sid && objective._sid == item._sid) {
      checked = "checked";
      sfm_code_idFlg = true;
    } else if (rowData && rowData.sfm_code_id == item._sid && !sfm_code_idFlg) {
      checked = "checked";
    }
...
function onRelateSfm_code_idBtn(e) {
  var selectedObj = document.querySelector('input[name="sfm_code_idName"]:checked');
  if (!selectedObj) {
    s_info(Strings.UTILITY_ALERT_TITLE, Strings.UTILITY_INVALID_SFM_CODE_ID, dlgOptions);
    return;
  }

  var objective = JSON.parse(selectedObj.getAttribute("sfm_code_idName").replaceAll('$DQUOTE$', '"'));
  utilitySfm_code_id.value = objective.sfm_code || "";
  utilitySfm_code_id.setAttribute("Sfm_code_id", objective._sid || "");
  selectedSfm_code_idId = objective._sid || "";
...
function onReasonZoomBtn(e) {
  ...
  var reasonFlg = false;
  var objective = {
    "id": utilityReason.getAttribute("Reason")
  };
  var check = true;
  if (UtilityInterface && UtilityInterface.onLookUpData) {
  ...
  for (var item of dataSources.getSfmReadingReasonCode) {
    ...
    if (objective.id && objective.id == item.id) {
      checked = "checked";
      reasonFlg = true;
    } else if (rowData && rowData.reason == item.id && !reasonFlg) {
      checked = "checked";
    }
...
function onRelateReasonBtn(e) {
  var selectedObj = document.querySelector('input[name="reasonName"]:checked');
  if (!selectedObj) {
    s_info(Strings.UTILITY_ALERT_TITLE, Strings.UTILITY_INVALID_REASON, dlgOptions);
    return;
  }

  var objective = JSON.parse(selectedObj.getAttribute("reasonName").replaceAll('$DQUOTE$', '"'));
  utilityReason.value = objective.reading_reason_code || "";
  utilityReason.setAttribute("Reason", objective.id || "");
  selectedReasonId = objective.id || "";
