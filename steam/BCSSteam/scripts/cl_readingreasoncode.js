/*
	...
	Version : 2024
	Generated On : Mon Mar 16 2026 12:34:25 GMT+0530 (India Standard Time)
	Description : Combine Master Ui for readingReasonCode
...
function onApplicationReady() {
  ...
  setMenu(document.getElementById('sideMenu'), 'bcssteam_readingReasonCode');
...
function initSheet() {
  ...
  editorSheet = jspreadsheet(sheetEl, {
    ...
    columns: [{
      ...
      {
        type: 'text',
        title: Strings.COL_LABEL_REASON_CODE,
        width: 400,
        align: 'left',
        wordWrap: true,
        readOnly: false
...
function registerEventListeners() {
  saveReadingReasonCodeBtn = document.getElementById('saveReadingReasonCodeBtn');
  saveReadingReasonCodeBtn.addEventListener('click', onSaveReadingReasonCodeBtnClick);
}
...
function onUser(resp) {
  ...
  clCommon.initLanguage(userObj.realm_lang, function() {
    ...
    if (typeof ReadingReasonCodeInterface === 'undefined')
      ReadingReasonCodeInterface = null;

    if (ReadingReasonCodeInterface && ReadingReasonCodeInterface.onLoad)
      ReadingReasonCodeInterface.onLoad();
    initControls();
...
function loadReadingReasonCodeData() {
  clearSheet();
  clCommon.showLoader("listProgress");

  var beforeLoad = true;
  if (ReadingReasonCodeInterface && ReadingReasonCodeInterface.onBeforeLoad)
    beforeLoad = ReadingReasonCodeInterface.onBeforeLoad();
  if (beforeLoad == false) {
    return;
  }

  Service.getSfmReadingReasonCode(function(resultObj) {
    ...
    if (resultObj.code == 200) { //On success response
      ...
      var dataArr = resultObj.data;
      if (ReadingReasonCodeInterface && ReadingReasonCodeInterface.onAfterLoad) {
        ...
        dataArr = ReadingReasonCodeInterface.onAfterLoad(isSuccessful, resultObj.data);
      }
      ...
      s_error(Strings.ERROR_TITLE, Strings.ERROR_LOAD_READINGREASONCODE, dlgOptions);
    }
  });
...
function onLoadSuccessResponse(dataArr) {
  if (dataArr.length > 0) {
    for (var i = 0; i < dataArr.length; i++) {
      ...
      editorSheet.setValue('B' + rowId, dataObj.reading_reason_code, true);
...
function onSaveReadingReasonCodeBtnClick(e) {
  var masterObj = {
    "master_data": [],
  ...
  for (var row = 0; row < sheetData.length; row++) {
    ...
    if (rowData[1] && rowData[1].length > 0) {
      ...
      // Assign values
      data.id = rowData[0];
      data.reading_reason_code = rowData[1];
      data.description = rowData[2];


      // Add internal row number for error reporting
      data._rowNum = rowNum;

      readingReasonCodeArr.push(data);
    }
  }

  var checkStatus = validate(readingReasonCodeArr);
  ...
  var finalData = [];
  readingReasonCodeArr.forEach(function(item) {
  ...
  var readingReasonCodeObj = true;
  if (ReadingReasonCodeInterface && ReadingReasonCodeInterface.onBeforeSave)
    readingReasonCodeObj = ReadingReasonCodeInterface.onBeforeSave(masterObj);
  if (readingReasonCodeObj == false) {
    return;
  }
  ...
  Service.saveSfmReadingReasonCode(masterObj, function(resultObj) {
    clCommon.hideLoader("listProgress");
    if (resultObj.code == 200) {
      var isSuccessful = true;

      var readingReasonCodeObj = true;
      if (ReadingReasonCodeInterface && ReadingReasonCodeInterface.onAfterSave)
        readingReasonCodeObj = ReadingReasonCodeInterface.onAfterSave(isSuccessful, resultObj.data);
      ...
      if (isSuccessful && resultObj.data.master_data) {
        try {
          ...
          s_error(Strings.$FULL_CAP_OBJNAME$_ERROR_TITLE, Strings.SAVE_$FULL_CAP_OBJNAME$_ERROR, dlgOptions);
        }
      }
      ...
      s_error(Strings.READINGREASONCODE_ERROR_TITLE, Strings.SAVE_READINGREASONCODE_ERROR, dlgOptions);
    }
  });
}
...
function validate(dataArr) {
  ...
  dataArr.forEach(function(readingReasonCodeObj) {
    var errorStr = "";
    var rowNum = readingReasonCodeObj._rowNum;

    function addError(error) {
      errorStr += error + "<br>";
    }

    var rowTitle = readingReasonCodeObj.name || readingReasonCodeObj.undefined || readingReasonCodeObj.id || "-";

    if (ReadingReasonCodeInterface && ReadingReasonCodeInterface.getRowTitle) {
      var customTitle = ReadingReasonCodeInterface.getRowTitle(readingReasonCodeObj, dataSources);
    ...
    if (readingReasonCodeObj.reading_reason_code == null || readingReasonCodeObj.reading_reason_code === '') {
      addError(Strings.READINGREASONCODE_INVALID_READING_REASON_CODE);
    }

    // C. External Interface Validation
    if (ReadingReasonCodeInterface && ReadingReasonCodeInterface.addValidation) {
      var extErr = ReadingReasonCodeInterface.addValidation(readingReasonCodeObj, dataSources);
  ...
  if (validateError.length > 0) {
    ...
    var promptTitle = (typeof Strings !== 'undefined' && Strings.VALIDATE_READINGREASONCODE_PROMPT) ? Strings.VALIDATE_READINGREASONCODE_PROMPT : "Validation Errors";
    var objTitle = (typeof Strings !== 'undefined' && Strings.READINGREASONCODE_TITLE) ? Strings.READINGREASONCODE_TITLE : "ReadingReasonCode";
...
function onSheetChangedListener(instance, cell, col, row, value) {
  ...
  var returnData = true;
  if (ReadingReasonCodeInterface && ReadingReasonCodeInterface.onSheetChanged)
    returnData = ReadingReasonCodeInterface.onSheetChanged(instance, cell, col, row, value);
  if (returnData == false) {
    return;
  }
