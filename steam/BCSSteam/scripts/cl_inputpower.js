/*
	...
	Version : 2024
	Generated On : Mon Mar 16 2026 11:13:20 GMT+0530 (India Standard Time)
...
function onSaveInputPowerBtnClick(e) {
  ...
  for (var row = 0; row < sheetData.length; row++) {
    ...
    if (rowData[1] && rowData[1].length > 0) {
      ...
      inputPowerArr.push(data);
    }
  }

  var checkStatus = validate(inputPowerArr);
  ...
  var finalData = [];
  inputPowerArr.forEach(function(item) {
...
function validate(dataArr) {
  ...
  dataArr.forEach(function(inputPowerObj) {
    var errorStr = "";
    var rowNum = inputPowerObj._rowNum;

    function addError(error) {
      errorStr += error + "<br>";
    }

    var rowTitle = inputPowerObj.name || inputPowerObj.undefined || inputPowerObj.id || "-";

    if (InputPowerInterface && InputPowerInterface.getRowTitle) {
      var customTitle = InputPowerInterface.getRowTitle(inputPowerObj, dataSources);
    ...
    if (inputPowerObj.input_power == null || inputPowerObj.input_power === '') {
      addError(Strings.INPUTPOWER_INVALID_INPUT_POWER);
    }

    // C. External Interface Validation
    if (InputPowerInterface && InputPowerInterface.addValidation) {
      var extErr = InputPowerInterface.addValidation(inputPowerObj, dataSources);
  ...
  if (validateError.length > 0) {
    ...
    var promptTitle = (typeof Strings !== 'undefined' && Strings.VALIDATE_INPUTPOWER_PROMPT) ? Strings.VALIDATE_INPUTPOWER_PROMPT : "Validation Errors";
    var objTitle = (typeof Strings !== 'undefined' && Strings.INPUTPOWER_TITLE) ? Strings.INPUTPOWER_TITLE : "InputPower";
