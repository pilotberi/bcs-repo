/*
	...
	Description : Get Service for connectivityOption
...
var getConnectOptionInterface = require("getinterface.js");
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.READ_BCSSTEAM_CONNECTIVITY_OPTION_GETCONNECTOPTION_SUCCESS
  };
  ...
  var onLoadRes = null;
  if (getConnectOptionInterface)
    onLoadRes = getConnectOptionInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (getConnectOptionInterface)
          respObj = getConnectOptionInterface.onError(respObj);
    ...
    if (getConnectOptionInterface)
      beforeDataRes = getConnectOptionInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params && params.id) {
        ...
        if (getConnectOptionInterface) {
          getParams = getConnectOptionInterface.beforeExecute(getParams);
        }
        //Sending request to ds client
        respObj = dsclientUtil.execute(getParams, "BCSSteam", "BCSSteam_connectivity_option", "get");
      } else {
        respObj.code = 400;
        respObj.msg = Strings.READ_BCSSTEAM_CONNECTIVITY_OPTION_ERROR;
      }
    }
    ...
    switch (respObj.code) {
      case 200:
        if (getConnectOptionInterface)
          respObj.data = getConnectOptionInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_BCSSTEAM_CONNECTIVITY_OPTION_GETCONNECTOPTION_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_BCSSTEAM_CONNECTIVITY_OPTION_GETCONNECTOPTION_ERROR;
        if (getConnectOptionInterface)
          respObj = getConnectOptionInterface.onError(respObj);
  ...
  if (getConnectOptionInterface)
    errorRespObj = getConnectOptionInterface.onError(errorRespObj);
