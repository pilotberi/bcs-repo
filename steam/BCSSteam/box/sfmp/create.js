/*
	...
	Description : Create Service for sfmp
...
var createSfmpInterface = require("createinterface.js");
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.CREATE_BCSSTEAM_SFMP_SUCCESS
  };
  ...
  var onLoadRes;
  if (createSfmpInterface) {
    onLoadRes = createSfmpInterface.onLoad(params);
  }
  // Check if 'onLoad' override is true
  if (createSfmpInterface && onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (createSfmpInterface) {
          respObj = createSfmpInterface.onError(respObj);
        }
        console.error("Unexpected server error when creating the sfmp - " + paramStr);
      }
    }
  } else {
    ...
    var beforeDataRes;
    if (createSfmpInterface) {
      beforeDataRes = createSfmpInterface.beforeData(params);
    }

    // Check if 'override' is true
    if (createSfmpInterface && beforeDataRes && beforeDataRes.override === true) {
      ...
      if (createSfmpInterface && beforeDataRes.params)
      ...
      if (params && params.data) {
        ...
        if (createSfmpInterface) {
          params = createSfmpInterface.beforeExecute(params);
        }
        //Sending request to ds client
        respObj = dsclientUtil.execute(params, "BCSSteam", "BCSSteam_sfmp", "insert");
      } else {
        respObj.code = 400;
        respObj.msg = Strings.CREATE_BCSSTEAM_SFMP_ERROR;
      }
    }
    ...
    switch (respObj.code) {
      case 200:
        if (createSfmpInterface) {
          respObj.data = createSfmpInterface.afterData(respObj.data);
        }
        respObj.msg = Strings.CREATE_BCSSTEAM_SFMP_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.CREATE_BCSSTEAM_SFMP_ERROR;
        if (createSfmpInterface) {
        ...
        console.error("Unable to create the sfmp - " + paramStr);
        break;
      case 403:
        ...
        console.error("User access denied when creating the sfmp - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (createSfmpInterface) {
  ...
  if (createSfmpInterface) {
    errorRespObj = createSfmpInterface.onError(errorRespObj);
  }
  console.error("Unexpected server error when creating the sfmp - " + paramStr + "-" + errorRespObj.message);
