/*
	...
	Description : Get Service for sfmp
...
var getSfmpInterface = require("getinterface.js");
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.READ_BCSSTEAM_SFMP_GETSFMP_SUCCESS
  };
  ...
  var onLoadRes = null;
  if (getSfmpInterface)
    onLoadRes = getSfmpInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (getSfmpInterface)
          respObj = getSfmpInterface.onError(respObj);
    ...
    if (getSfmpInterface)
      beforeDataRes = getSfmpInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params && params.id) {
        ...
        if (getSfmpInterface) {
          getParams = getSfmpInterface.beforeExecute(getParams);
        }
        //Sending request to ds client
        respObj = dsclientUtil.execute(getParams, "BCSSteam", "BCSSteam_sfmp", "get");
      } else {
        respObj.code = 400;
        respObj.msg = Strings.READ_BCSSTEAM_SFMP_ERROR;
      }
    }
    ...
    switch (respObj.code) {
      case 200:
        if (getSfmpInterface)
          respObj.data = getSfmpInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_BCSSTEAM_SFMP_GETSFMP_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_BCSSTEAM_SFMP_GETSFMP_ERROR;
        if (getSfmpInterface)
          respObj = getSfmpInterface.onError(respObj);
  ...
  if (getSfmpInterface)
    errorRespObj = getSfmpInterface.onError(errorRespObj);
