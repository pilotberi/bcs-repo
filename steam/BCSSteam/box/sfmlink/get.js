/*
	...
	Description : Get Service for sfmLink
...
var getSfmLinkInterface = require("getinterface.js");
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.READ_BCSSTEAM_SFM_LINK_GETSFMLINK_SUCCESS
  };
  ...
  var onLoadRes = null;
  if (getSfmLinkInterface)
    onLoadRes = getSfmLinkInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (getSfmLinkInterface)
          respObj = getSfmLinkInterface.onError(respObj);
    ...
    if (getSfmLinkInterface)
      beforeDataRes = getSfmLinkInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params && params.id) {
        ...
        if (getSfmLinkInterface) {
          getParams = getSfmLinkInterface.beforeExecute(getParams);
        }
        //Sending request to ds client
        respObj = dsclientUtil.execute(getParams, "BCSSteam", "BCSSteam_sfm_link", "get");
      } else {
        respObj.code = 400;
        respObj.msg = Strings.READ_BCSSTEAM_SFM_LINK_ERROR;
      }
    }
    ...
    switch (respObj.code) {
      case 200:
        if (getSfmLinkInterface)
          respObj.data = getSfmLinkInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_BCSSTEAM_SFM_LINK_GETSFMLINK_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_BCSSTEAM_SFM_LINK_GETSFMLINK_ERROR;
        if (getSfmLinkInterface)
          respObj = getSfmLinkInterface.onError(respObj);
  ...
  if (getSfmLinkInterface)
    errorRespObj = getSfmLinkInterface.onError(errorRespObj);
