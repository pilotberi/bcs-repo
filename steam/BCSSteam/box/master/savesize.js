/*
	...
	Version : 2024
	Generated On : Mon Jan 05 2026 12:35:12 GMT+0530 (India Standard Time)
	Description : CombineMaster Save Service for size
...
var saveSizeInterface = require("savesizeinterface.js");
...
try {
  ...
  var onLoadRes = null;
  if (saveSizeInterface && saveSizeInterface.onLoad)
    onLoadRes = saveSizeInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (saveSizeInterface && saveSizeInterface.onError)
          respObj = saveSizeInterface.onError(respObj);
    ...
    if (saveSizeInterface && saveSizeInterface.beforeData)
      beforeDataRes = saveSizeInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params && params.data) {
        ...
        if (saveSizeInterface && saveSizeInterface.beforeExecute)
          queryParams = saveSizeInterface.beforeExecute(queryParams, "query");
        ...
        if (masterRespObj) {
          ...
          if (masterObj) { //master already has data, new data should be appended
            ...
            if (saveSizeInterface && saveSizeInterface.beforeExecute)
              masterDataStr = saveSizeInterface.beforeExecute(masterDataStr, "update");
            ...
            if (saveSizeInterface && saveSizeInterface.beforeExecute)
              masterDataStr = saveSizeInterface.beforeExecute(masterDataStr, "insert");
    ...
    switch (respObj.code) {
      case 200:
        if (saveSizeInterface && saveSizeInterface.afterData)
          respObj.data = saveSizeInterface.afterData(respObj.data);
      ...
      case 400:
        ...
        if (saveSizeInterface && saveSizeInterface.onError)
          respObj = saveSizeInterface.onError(respObj);
        console.error("Unable to save the 'size' - " + paramStr);
        break;
      case 403:
        respObj.msg = Strings.ACCESS_DENIED;
        if (saveSizeInterface && saveSizeInterface.onError)
          respObj = saveSizeInterface.onError(respObj);
        console.error("User access denied when saving the 'size' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (saveSizeInterface && saveSizeInterface.onError)
          respObj = saveSizeInterface.onError(respObj);
        console.error("Unexpected server error when saving the 'size' - " + paramStr);
        break;
    }
  ...
  if (saveSizeInterface && saveSizeInterface.onError)
    errorRespObj = saveSizeInterface.onError(errorRespObj);
  console.error("Unexpected server error when saving the size - " + paramStr + "-" + errorRespObj.message);
