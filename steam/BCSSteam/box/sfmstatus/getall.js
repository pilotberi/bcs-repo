/*
	...
	Description : Get All Service for sfmStatus
...
var getAllSfmStatusInterface = require("getallinterface.js");
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.READ_BCSSTEAM_SFM_STATUS_GETALLSFMSTATUS_SUCCESS
  };
  ...
  var onLoadRes = null;
  if (getAllSfmStatusInterface)
    onLoadRes = getAllSfmStatusInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (getAllSfmStatusInterface)
          respObj = getAllSfmStatusInterface.onError(respObj);
        console.error("Unexpected server error when retrieving the sfmStatus - " + paramStr);
      }
    }
  } else {
    ...
    if (getAllSfmStatusInterface)
      beforeDataRes = getAllSfmStatusInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (getAllSfmStatusInterface) {
        params = getAllSfmStatusInterface.beforeExecute(params);
      }

      var countRespObj = null;

      var getAllSfmStatusQueryParams = {};


      //Setting offset and limit after count query
      if (params.offset)
        getAllSfmStatusQueryParams.offset = params.offset;

      if (params.limit)
        getAllSfmStatusQueryParams.limit = params.limit;

      if (params.offset || params.limit)
        countRespObj = dsclientUtil.execute(getAllSfmStatusQueryParams, "BCSSteam", "BCSSteam_sfm_status", "count");

      //Sending request to ds client
      respObj = dsclientUtil.execute(params, "BCSSteam", "BCSSteam_sfm_status", "getAll");
    ...
    switch (respObj.code) {
      case 200:
        if (getAllSfmStatusInterface)
          respObj.data = getAllSfmStatusInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_BCSSTEAM_SFM_STATUSES_GETALLSFMSTATUS_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_BCSSTEAM_SFM_STATUSES_GETALLSFMSTATUS_ERROR;
        if (getAllSfmStatusInterface)
          respObj = getAllSfmStatusInterface.onError(respObj);
        console.error("Unable to retrieve the sfmStatus - " + paramStr);
        break;
      case 403:
        respObj.msg = Strings.ACCESS_DENIED;
        if (getAllSfmStatusInterface)
          respObj = getAllSfmStatusInterface.onError(respObj);
        console.error("User access denied when retrieving the sfmStatus - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (getAllSfmStatusInterface)
  ...
  if (getAllSfmStatusInterface)
    errorRespObj = getAllSfmStatusInterface.onError(errorRespObj);
  console.error("Unexpected server error when retrieving the sfmStatus - " + paramStr + "-" + errorRespObj.message);
