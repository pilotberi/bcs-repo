/*
	...
	Description : Get Service for utility
...
var getUtiliyInterface = require("getinterface.js");
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.READ_BCSSTEAM_UTILITY_GETUTILIY_SUCCESS
  };
  ...
  var onLoadRes = null;
  if (getUtiliyInterface)
    onLoadRes = getUtiliyInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (getUtiliyInterface)
          respObj = getUtiliyInterface.onError(respObj);
    ...
    if (getUtiliyInterface)
      beforeDataRes = getUtiliyInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params && params.id) {
        ...
        if (getUtiliyInterface) {
          getParams = getUtiliyInterface.beforeExecute(getParams);
        }
        //Sending request to ds client
        respObj = dsclientUtil.execute(getParams, "BCSSteam", "BCSSteam_utility", "get");
      } else {
        respObj.code = 400;
    ...
    switch (respObj.code) {
      case 200:
        if (getUtiliyInterface)
          respObj.data = getUtiliyInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_BCSSTEAM_UTILITY_GETUTILIY_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_BCSSTEAM_UTILITY_GETUTILIY_ERROR;
        if (getUtiliyInterface)
          respObj = getUtiliyInterface.onError(respObj);
  ...
  if (getUtiliyInterface)
    errorRespObj = getUtiliyInterface.onError(errorRespObj);
