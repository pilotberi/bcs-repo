/*
	...
	Description : Update Service for sfm
...
var updateSfmInterface = require("updateinterface.js");
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.UPDATE_BCSSTEAM_SFM_SUCCESS
  };
  ...
  var onLoadRes;
  if (updateSfmInterface)
    onLoadRes = updateSfmInterface.onLoad(params);

  // Check if 'onLoad' override is true
  if (updateSfmInterface && onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (updateSfmInterface)
          respObj = updateSfmInterface.onError(respObj);
        console.error("Unexpected server error when updating the sfm - " + paramStr);
      }
    }
  } else {
    ...
    var beforeDataRes;
    if (updateSfmInterface)
      beforeDataRes = updateSfmInterface.beforeData(params);

    // Check if 'override' is true
    if (updateSfmInterface && beforeDataRes && beforeDataRes.override === true) {
      ...
      if (updateSfmInterface && beforeDataRes.params)
      ...
      if (params && params.data) {
        ...
        if (updateSfmInterface) {
          params = updateSfmInterface.beforeExecute(params);
        }
        //Sending request to ds client
        respObj = dsclientUtil.execute(params, "BCSSteam", "BCSSteam_sfm", "update");
      } else {
        respObj.code = 400;
        respObj.msg = Strings.UPDATE_BCSSTEAM_SFM_ERROR;
      }
    }
    ...
    switch (respObj.code) {
      case 200:
        if (updateSfmInterface)
          respObj.data = updateSfmInterface.afterData(respObj.data);
        respObj.msg = Strings.UPDATE_BCSSTEAM_SFM_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.UPDATE_BCSSTEAM_SFM_ERROR;
        if (updateSfmInterface)
          respObj = updateSfmInterface.onError(respObj);
        console.error("Unable to update the sfm - " + paramStr);
        break;
      case 403:
        respObj.msg = Strings.ACCESS_DENIED;
        if (updateSfmInterface)
          respObj = updateSfmInterface.onError(respObj);
        console.error("User access denied when updating the sfm - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (updateSfmInterface)
  ...
  if (updateSfmInterface)
    errorRespObj = updateSfmInterface.onError(errorRespObj);
  console.error("Unexpected server error when updating the sfm - " + paramStr + "-" + errorRespObj.message);
