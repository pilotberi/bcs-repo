/*
		...
		Version : 2024
		Generated On : Mon Jan 05 2026 12:18:48 GMT+0530 (India Standard Time)
		Description : CombineMaster Save Service Interface for workingPrinciple
...
var TenantSaveWorkingPrincipleInterface = app.getTenantWSInterface('BCSSteam', 'master/saveworkingprinciple');

if (typeof TenantSaveWorkingPrincipleInterface === 'undefined')
	TenantSaveWorkingPrincipleInterface = {};
...
module.exports = {
	onLoad: function(params) {
		...
		if (TenantSaveWorkingPrincipleInterface && TenantSaveWorkingPrincipleInterface.onLoad) {
			rObj = TenantSaveWorkingPrincipleInterface.onLoad(params);
		}
	...
	beforeData: function(params) {
		...
		if (masterRespObj && masterRespObj.code == 200 && masterRespObj.data && masterRespObj.data.length > 0) {
			...
			wpDataArr = JSON.parse(masterRespObj.data[0].master_data);

			//taking deleted obj
			for (var j = 0; j < wpDataArr.length; j++) {
				var found = false;
				var singleObj = wpDataArr[j];
			...
			if (deletedObjectsArr.length > 0) {
				...
				for(var paramIdx = 0; paramIdx < paramArr.length; paramIdx++){
					for(var dataIdx = 0; dataIdx < deletedObjectsArr.length; dataIdx++){
						if(paramArr[paramIdx].id == null && normalize( paramArr[paramIdx].working_principle) == normalize(deletedObjectsArr[dataIdx].working_principle)){
				...
				var getBySfmByWp = {
					"cols": ["working_principle_id"],
					"filter": [{
						"col": "working_principle_id",
						"op": "IN",
						"val": formatedDeletedIds
					}]
				};

				var sfmRespObj = dsclientUtil.execute(getBySfmByWp, "BCSSteam", "BCSSteam_sfm", "query");
				...
				if (sfmRespObj.code === 200 && sfmObjArr && sfmObjArr.length > 0) {
					...
					for (var idx = 0; idx < deletedObjectsArr.length; idx++) {
						for (var jdx = 0; jdx < sfmObjArr.length; jdx++) {
							if (deletedObjectsArr[idx].id == sfmObjArr[jdx].working_principle_id) {
		...
		if (TenantSaveWorkingPrincipleInterface && TenantSaveWorkingPrincipleInterface.beforeData) {
			rObj = TenantSaveWorkingPrincipleInterface.beforeData(params);
		}
	...
	beforeExecute: function(queryParams, qType) {
		...
		if (TenantSaveWorkingPrincipleInterface && TenantSaveWorkingPrincipleInterface.beforeExecute) {
			queryParams = TenantSaveWorkingPrincipleInterface.beforeExecute(queryParams, qType);
		}
		return queryParams;
	},

	afterData: function(saveWorkingPrincipleData) {


		if (TenantSaveWorkingPrincipleInterface && TenantSaveWorkingPrincipleInterface.afterData) {
			saveWorkingPrincipleData = TenantSaveWorkingPrincipleInterface.afterData(saveWorkingPrincipleData);
		}
		return saveWorkingPrincipleData;
	},

	onError: function(errorObj) {

		if (TenantSaveWorkingPrincipleInterface && TenantSaveWorkingPrincipleInterface.onError) {
			errorObj = TenantSaveWorkingPrincipleInterface.onError(errorObj);
		}
		return errorObj;
	},
