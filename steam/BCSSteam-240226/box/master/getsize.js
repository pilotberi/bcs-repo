/*
	...
	Description : Get Service for size
...
var getSizeInterface = require("getsizeinterface.js");
...
function setDefaultMasterData(defaultArr) {
  ...
  var respObj = {
    code: 200,
    msg: Strings.UPDATE_SIZE_SUCCESS
  };

  var MASTER_TABLE = "bcssteam_size";
  ...
  if (getSizeInterface && getSizeInterface.beforeExecute)
    queryParams = getSizeInterface.beforeExecute(queryParams, "query");
  ...
  if (masterRespObj) {
    ...
    if (masterObj) { //master already has data, new data should be appended
      ...
      if (getSizeInterface && getSizeInterface.beforeExecute)
        masterDataStr = getSizeInterface.beforeExecute(masterDataStr, "update");
      ...
      if (getSizeInterface && getSizeInterface.beforeExecute)
        masterDataStr = getSizeInterface.beforeExecute(masterDataStr, "insert");
    ...
    switch (respObj.code) {
      case 200:
        if (getSizeInterface && getSizeInterface.onAfterData)
          respObj.data = getSizeInterface.onAfterData(respObj.data);
        respObj.msg = Strings.UPDATE_SIZE_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.UPDATE_SIZE_ERROR;
        if (getSizeInterface && getSizeInterface.onError)
          respObj = getSizeInterface.onError(respObj);
        console.error("Unable to create the 'size' - " + paramStr);
        break;
      case 403:
        respObj.msg = Strings.ACCESS_DENIED;
        if (getSizeInterface && getSizeInterface.onError)
          respObj = getSizeInterface.onError(respObj);
        console.error("User access denied when creating the 'size' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (getSizeInterface && getSizeInterface.onError)
          respObj = getSizeInterface.onError(respObj);
        console.error("Unexpected server error when creating the 'size' - " + paramStr);
        break;
    }
...
try {
  ...
  var onLoadRes = null;
  if (getSizeInterface && getSizeInterface.onLoad)
    onLoadRes = getSizeInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (getSizeInterface && getSizeInterface.onError)
          respObj = getSizeInterface.onError(respObj);
        console.error("Unexpected server error when retrieving the size - " + paramStr);
      }
    }
  } else {
    ...
    if (getSizeInterface && getSizeInterface.beforeData)
      beforeDataRes = getSizeInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params) {
        ...
        respObj.code = 400;
        respObj.msg = Strings.READ_SIZE_ERROR;
        if (getSizeInterface && getSizeInterface.onError)
          respObj = getSizeInterface.onError(respObj);
      }
    }
    ...
    switch (respObj.code) {
      case 200:
        if (getSizeInterface && getSizeInterface.afterData)
          respObj.data = getSizeInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_SIZE_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_SIZE_ERROR;
        if (getSizeInterface && getSizeInterface.onError)
      ...
      case 403:
        ...
        if (getSizeInterface && getSizeInterface.onError)
          respObj = getSizeInterface.onError(respObj);
        console.error("User access denied when retrieving the 'size' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (getSizeInterface && getSizeInterface.onError)
  ...
  if (getSizeInterface && getSizeInterface.onError)
    errorRespObj = getSizeInterface.onError(errorRespObj);
  console.error("Unexpected server error when creating the 'size' - " + paramStr + "-" + errorRespObj.message);
