/*
	...
	Description : CombineMaster Save Service for inputPower
...
var saveInputPowerInterface = require("saveinputpowerinterface.js");
...
try {
  ...
  var onLoadRes = null;
  if (saveInputPowerInterface && saveInputPowerInterface.onLoad)
    onLoadRes = saveInputPowerInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (saveInputPowerInterface && saveInputPowerInterface.onError)
          respObj = saveInputPowerInterface.onError(respObj);
    ...
    if (saveInputPowerInterface && saveInputPowerInterface.beforeData)
      beforeDataRes = saveInputPowerInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params && params.data) {
        ...
        if (saveInputPowerInterface && saveInputPowerInterface.beforeExecute)
          queryParams = saveInputPowerInterface.beforeExecute(queryParams, "query");
        ...
        if (masterRespObj) {
          ...
          if (masterObj) { //master already has data, new data should be appended
            ...
            if (saveInputPowerInterface && saveInputPowerInterface.beforeExecute)
              masterDataStr = saveInputPowerInterface.beforeExecute(masterDataStr, "update");
            ...
            if (saveInputPowerInterface && saveInputPowerInterface.beforeExecute)
              masterDataStr = saveInputPowerInterface.beforeExecute(masterDataStr, "insert");
    ...
    switch (respObj.code) {
      case 200:
        if (saveInputPowerInterface && saveInputPowerInterface.afterData)
          respObj.data = saveInputPowerInterface.afterData(respObj.data);
      ...
      case 400:
        ...
        if (saveInputPowerInterface && saveInputPowerInterface.onError)
          respObj = saveInputPowerInterface.onError(respObj);
        console.error("Unable to save the 'inputPower' - " + paramStr);
        break;
      case 403:
        respObj.msg = Strings.ACCESS_DENIED;
        if (saveInputPowerInterface && saveInputPowerInterface.onError)
          respObj = saveInputPowerInterface.onError(respObj);
        console.error("User access denied when saving the 'inputPower' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (saveInputPowerInterface && saveInputPowerInterface.onError)
          respObj = saveInputPowerInterface.onError(respObj);
        console.error("Unexpected server error when saving the 'inputPower' - " + paramStr);
        break;
    }
  ...
  if (saveInputPowerInterface && saveInputPowerInterface.onError)
    errorRespObj = saveInputPowerInterface.onError(errorRespObj);
  console.error("Unexpected server error when saving the inputPower - " + paramStr + "-" + errorRespObj.message);
