...
var UtilityInterface = {
	...
	onBeforeLoad: function(dataSources) {
		/*var dateCookieObj = clCommon.getCookie("dates");
		if (dateCookieObj) {
			dateCookieObj = JSON.parse(dateCookieObj);
			dateFilter.value = dateCookieObj.dateFilter;
			if (dateCookieObj.sDate && dateCookieObj.eDate) {
				startDate.value = getFormattedDate(dateCookieObj.sDate);
				endDate.value = getFormattedDate(dateCookieObj.eDate);
			} else {
...
function convertDateToMonthYearPicker() {
	...
	if (sfmContainer) {
		...
		label.innerHTML = 'MM/YYYY<sup>*</sup>';
