/*
	...
	Description : CombineMaster Save Service Interface for size
...
var TenantSaveSizeInterface = app.getTenantWSInterface('BCSSteam', 'master/savesize');

if (typeof TenantSaveSizeInterface === 'undefined')
	TenantSaveSizeInterface = {};
...
module.exports = {
	onLoad: function(params) {
		...
		if (TenantSaveSizeInterface && TenantSaveSizeInterface.onLoad) {
			rObj = TenantSaveSizeInterface.onLoad(params);
		}
	...
	beforeData: function(params) {
		...
		if (masterRespObj && masterRespObj.code == 200 && masterRespObj.data && masterRespObj.data.length > 0) {
			...
			sizeDataArr = JSON.parse(masterRespObj.data[0].master_data);

			//taking deleted obj
			for (var j = 0; j < sizeDataArr.length; j++) {
				var found = false;
				var singleObj = sizeDataArr[j];
			...
			if (deletedObjectsArr.length > 0) {
				...
				for(var paramIdx = 0; paramIdx < paramArr.length; paramIdx++){
					for(var dataIdx = 0; dataIdx < deletedObjectsArr.length; dataIdx++){
						if(paramArr[paramIdx].id == null && normalize( paramArr[paramIdx].size) == normalize(deletedObjectsArr[dataIdx].size)){
				...
				var sfmRespObj = dsclientUtil.execute(getSfmBySize, "BCSSteam", "BCSSteam_sfm", "query");
				...
				if (sfmRespObj.code === 200 && sfmObjArr && sfmObjArr.length > 0) {
					...
					for (var idx = 0; idx < deletedObjectsArr.length; idx++) {
						for (var jdx = 0; jdx < sfmObjArr.length; jdx++) {
							if (deletedObjectsArr[idx].id == sfmObjArr[jdx].size_id) {
		...
		if (TenantSaveSizeInterface && TenantSaveSizeInterface.beforeData) {
			rObj = TenantSaveSizeInterface.beforeData(params);
		}
	...
	beforeExecute: function(queryParams, qType) {
		...
		if (TenantSaveSizeInterface && TenantSaveSizeInterface.beforeExecute) {
			queryParams = TenantSaveSizeInterface.beforeExecute(queryParams, qType);
		}
		return queryParams;
	},

	afterData: function(saveSizeData) {


		if (TenantSaveSizeInterface && TenantSaveSizeInterface.afterData) {
			saveSizeData = TenantSaveSizeInterface.afterData(saveSizeData);
		}
		return saveSizeData;
	},
	...
	onError: function(errorObj) {
		...
		if (TenantSaveSizeInterface && TenantSaveSizeInterface.onError) {
			errorObj = TenantSaveSizeInterface.onError(errorObj);
		}
		return errorObj;
	}
};
