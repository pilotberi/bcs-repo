/*
	...
	Description : Get Service for outputSignal
...
var getOutputSignalInterface = require("getoutputsignalinterface.js");
...
function setDefaultMasterData(defaultArr) {
  ...
  if (getOutputSignalInterface && getOutputSignalInterface.beforeExecute)
    queryParams = getOutputSignalInterface.beforeExecute(queryParams, "query");
  ...
  if (masterRespObj) {
    ...
    if (masterObj) { //master already has data, new data should be appended
      ...
      if (getOutputSignalInterface && getOutputSignalInterface.beforeExecute)
        masterDataStr = getOutputSignalInterface.beforeExecute(masterDataStr, "update");
      ...
      if (getOutputSignalInterface && getOutputSignalInterface.beforeExecute)
        masterDataStr = getOutputSignalInterface.beforeExecute(masterDataStr, "insert");
    ...
    switch (respObj.code) {
      case 200:
        if (getOutputSignalInterface && getOutputSignalInterface.onAfterData)
          respObj.data = getOutputSignalInterface.onAfterData(respObj.data);
        respObj.msg = Strings.UPDATE_OUTPUTSIGNAL_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.UPDATE_OUTPUTSIGNAL_ERROR;
        if (getOutputSignalInterface && getOutputSignalInterface.onError)
          respObj = getOutputSignalInterface.onError(respObj);
        console.error("Unable to create the 'outputSignal' - " + paramStr);
        break;
      case 403:
        respObj.msg = Strings.ACCESS_DENIED;
        if (getOutputSignalInterface && getOutputSignalInterface.onError)
          respObj = getOutputSignalInterface.onError(respObj);
        console.error("User access denied when creating the 'outputSignal' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (getOutputSignalInterface && getOutputSignalInterface.onError)
          respObj = getOutputSignalInterface.onError(respObj);
        console.error("Unexpected server error when creating the 'outputSignal' - " + paramStr);
        break;
    }
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.READ_OUTPUTSIGNAL_SUCCESS
  };
  ...
  var onLoadRes = null;
  if (getOutputSignalInterface && getOutputSignalInterface.onLoad)
    onLoadRes = getOutputSignalInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    ...
    if (getOutputSignalInterface && getOutputSignalInterface.beforeData)
      beforeDataRes = getOutputSignalInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params) {
        ...
        respObj.code = 400;
        respObj.msg = Strings.READ_OUTPUTSIGNAL_ERROR;
        if (getOutputSignalInterface && getOutputSignalInterface.onError)
          respObj = getOutputSignalInterface.onError(respObj);
      }
    }
    ...
    switch (respObj.code) {
      case 200:
        if (getOutputSignalInterface && getOutputSignalInterface.afterData)
          respObj.data = getOutputSignalInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_OUTPUTSIGNAL_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_OUTPUTSIGNAL_ERROR;
        if (getOutputSignalInterface && getOutputSignalInterface.onError)
      ...
      case 403:
        ...
        if (getOutputSignalInterface && getOutputSignalInterface.onError)
          respObj = getOutputSignalInterface.onError(respObj);
        console.error("User access denied when retrieving the 'outputSignal' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (getOutputSignalInterface && getOutputSignalInterface.onError)
  ...
  if (getOutputSignalInterface && getOutputSignalInterface.onError)
    errorRespObj = getOutputSignalInterface.onError(errorRespObj);
  console.error("Unexpected server error when creating the 'outputSignal' - " + paramStr + "-" + errorRespObj.message);
