/*
	...
	Description : CombineMaster Save Service for code
...
var saveCodeInterface = require("savecodeinterface.js");
...
try {
  ...
  var onLoadRes = null;
  if (saveCodeInterface && saveCodeInterface.onLoad)
    onLoadRes = saveCodeInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (saveCodeInterface && saveCodeInterface.onError)
          respObj = saveCodeInterface.onError(respObj);
    ...
    if (saveCodeInterface && saveCodeInterface.beforeData)
      beforeDataRes = saveCodeInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params && params.data) {
        ...
        if (saveCodeInterface && saveCodeInterface.beforeExecute)
          queryParams = saveCodeInterface.beforeExecute(queryParams, "query");
        ...
        if (masterRespObj) {
          ...
          if (masterObj) { //master already has data, new data should be appended
            ...
            if (saveCodeInterface && saveCodeInterface.beforeExecute)
              masterDataStr = saveCodeInterface.beforeExecute(masterDataStr, "update");
            ...
            if (saveCodeInterface && saveCodeInterface.beforeExecute)
              masterDataStr = saveCodeInterface.beforeExecute(masterDataStr, "insert");
    ...
    switch (respObj.code) {
      case 200:
        if (saveCodeInterface && saveCodeInterface.afterData)
          respObj.data = saveCodeInterface.afterData(respObj.data);
      ...
      case 400:
        ...
        if (saveCodeInterface && saveCodeInterface.onError)
          respObj = saveCodeInterface.onError(respObj);
        console.error("Unable to save the 'code' - " + paramStr);
        break;
      case 403:
        respObj.msg = Strings.ACCESS_DENIED;
        if (saveCodeInterface && saveCodeInterface.onError)
          respObj = saveCodeInterface.onError(respObj);
        console.error("User access denied when saving the 'code' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (saveCodeInterface && saveCodeInterface.onError)
          respObj = saveCodeInterface.onError(respObj);
        console.error("Unexpected server error when saving the 'code' - " + paramStr);
        break;
    }
  ...
  if (saveCodeInterface && saveCodeInterface.onError)
    errorRespObj = saveCodeInterface.onError(errorRespObj);
  console.error("Unexpected server error when saving the code - " + paramStr + "-" + errorRespObj.message);
