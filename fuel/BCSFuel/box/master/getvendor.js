/*
	...
	Version : 2024
	Generated On : Mon Jan 26 2026 19:54:40 GMT+0530 (India Standard Time)
...
var getVendorInterface = require("getvendorinterface.js");
...
function setDefaultMasterData(defaultArr) {
  ...
  var respObj = {
    code: 200,
    msg: Strings.UPDATE_VENDOR_SUCCESS
  };
  ...
  if (getVendorInterface && getVendorInterface.beforeExecute)
    queryParams = getVendorInterface.beforeExecute(queryParams, "query");
  ...
  if (masterRespObj) {
    ...
    if (masterObj) { //master already has data, new data should be appended
      ...
      if (getVendorInterface && getVendorInterface.beforeExecute)
        masterDataStr = getVendorInterface.beforeExecute(masterDataStr, "update");
      ...
      if (getVendorInterface && getVendorInterface.beforeExecute)
        masterDataStr = getVendorInterface.beforeExecute(masterDataStr, "insert");
    ...
    switch (respObj.code) {
      case 200:
        if (getVendorInterface && getVendorInterface.onAfterData)
          respObj.data = getVendorInterface.onAfterData(respObj.data);
        respObj.msg = Strings.UPDATE_VENDOR_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.UPDATE_VENDOR_ERROR;
        if (getVendorInterface && getVendorInterface.onError)
          respObj = getVendorInterface.onError(respObj);
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.READ_VENDOR_SUCCESS
  };
  ...
  var onLoadRes = null;
  if (getVendorInterface && getVendorInterface.onLoad)
    onLoadRes = getVendorInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    ...
    if (getVendorInterface && getVendorInterface.beforeData)
      beforeDataRes = getVendorInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params) {
        ...
        respObj.code = 400;
        respObj.msg = Strings.READ_VENDOR_ERROR;
        if (getVendorInterface && getVendorInterface.onError)
          respObj = getVendorInterface.onError(respObj);
      }
    }
    ...
    switch (respObj.code) {
      case 200:
        if (getVendorInterface && getVendorInterface.afterData)
          respObj.data = getVendorInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_VENDOR_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_VENDOR_ERROR;
        if (getVendorInterface && getVendorInterface.onError)
  ...
  if (getVendorInterface && getVendorInterface.onError)
    errorRespObj = getVendorInterface.onError(errorRespObj);
