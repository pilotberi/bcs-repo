/*
	...
	Description : Get Service for unit
...
var getUnitInterface = require("getunitinterface.js");
...
function setDefaultMasterData(defaultArr) {
  ...
  var respObj = {
    code: 200,
    msg: Strings.UPDATE_UNIT_SUCCESS
  };
  ...
  if (getUnitInterface && getUnitInterface.beforeExecute)
    queryParams = getUnitInterface.beforeExecute(queryParams, "query");
  ...
  if (masterRespObj) {
    ...
    if (masterObj) { //master already has data, new data should be appended
      ...
      if (getUnitInterface && getUnitInterface.beforeExecute)
        masterDataStr = getUnitInterface.beforeExecute(masterDataStr, "update");
      ...
      if (getUnitInterface && getUnitInterface.beforeExecute)
        masterDataStr = getUnitInterface.beforeExecute(masterDataStr, "insert");
    ...
    switch (respObj.code) {
      case 200:
        if (getUnitInterface && getUnitInterface.onAfterData)
          respObj.data = getUnitInterface.onAfterData(respObj.data);
        respObj.msg = Strings.UPDATE_UNIT_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.UPDATE_UNIT_ERROR;
        if (getUnitInterface && getUnitInterface.onError)
          respObj = getUnitInterface.onError(respObj);
        console.error("Unable to create the 'unit' - " + paramStr);
        break;
      case 403:
        respObj.msg = Strings.ACCESS_DENIED;
        if (getUnitInterface && getUnitInterface.onError)
          respObj = getUnitInterface.onError(respObj);
        console.error("User access denied when creating the 'unit' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (getUnitInterface && getUnitInterface.onError)
          respObj = getUnitInterface.onError(respObj);
        console.error("Unexpected server error when creating the 'unit' - " + paramStr);
        break;
    }
...
try {
  ...
  var onLoadRes = null;
  if (getUnitInterface && getUnitInterface.onLoad)
    onLoadRes = getUnitInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (getUnitInterface && getUnitInterface.onError)
          respObj = getUnitInterface.onError(respObj);
        console.error("Unexpected server error when retrieving the unit - " + paramStr);
      }
    }
  } else {
    ...
    if (getUnitInterface && getUnitInterface.beforeData)
      beforeDataRes = getUnitInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params) {
        ...
        respObj.code = 400;
        respObj.msg = Strings.READ_UNIT_ERROR;
        if (getUnitInterface && getUnitInterface.onError)
          respObj = getUnitInterface.onError(respObj);
      }
    }
    ...
    switch (respObj.code) {
      case 200:
        if (getUnitInterface && getUnitInterface.afterData)
          respObj.data = getUnitInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_UNIT_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_UNIT_ERROR;
        if (getUnitInterface && getUnitInterface.onError)
      ...
      case 403:
        ...
        if (getUnitInterface && getUnitInterface.onError)
          respObj = getUnitInterface.onError(respObj);
        console.error("User access denied when retrieving the 'unit' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (getUnitInterface && getUnitInterface.onError)
  ...
  if (getUnitInterface && getUnitInterface.onError)
    errorRespObj = getUnitInterface.onError(errorRespObj);
  console.error("Unexpected server error when creating the 'unit' - " + paramStr + "-" + errorRespObj.message);
