/*
	...
	Description : CombineMaster Save Service for unit
...
var saveUnitInterface = require("saveunitinterface.js");
...
try {
  ...
  var onLoadRes = null;
  if (saveUnitInterface && saveUnitInterface.onLoad)
    onLoadRes = saveUnitInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (saveUnitInterface && saveUnitInterface.onError)
          respObj = saveUnitInterface.onError(respObj);
    ...
    if (saveUnitInterface && saveUnitInterface.beforeData)
      beforeDataRes = saveUnitInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params && params.data) {
        ...
        if (saveUnitInterface && saveUnitInterface.beforeExecute)
          queryParams = saveUnitInterface.beforeExecute(queryParams, "query");
        ...
        if (masterRespObj) {
          ...
          if (masterObj) { //master already has data, new data should be appended
            ...
            if (saveUnitInterface && saveUnitInterface.beforeExecute)
              masterDataStr = saveUnitInterface.beforeExecute(masterDataStr, "update");
            ...
            if (saveUnitInterface && saveUnitInterface.beforeExecute)
              masterDataStr = saveUnitInterface.beforeExecute(masterDataStr, "insert");
    ...
    switch (respObj.code) {
      case 200:
        if (saveUnitInterface && saveUnitInterface.afterData)
          respObj.data = saveUnitInterface.afterData(respObj.data);
      ...
      case 400:
        ...
        if (saveUnitInterface && saveUnitInterface.onError)
          respObj = saveUnitInterface.onError(respObj);
        console.error("Unable to save the 'unit' - " + paramStr);
        break;
      case 403:
        respObj.msg = Strings.ACCESS_DENIED;
        if (saveUnitInterface && saveUnitInterface.onError)
          respObj = saveUnitInterface.onError(respObj);
        console.error("User access denied when saving the 'unit' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (saveUnitInterface && saveUnitInterface.onError)
          respObj = saveUnitInterface.onError(respObj);
        console.error("Unexpected server error when saving the 'unit' - " + paramStr);
        break;
    }
  ...
  if (saveUnitInterface && saveUnitInterface.onError)
    errorRespObj = saveUnitInterface.onError(errorRespObj);
  console.error("Unexpected server error when saving the unit - " + paramStr + "-" + errorRespObj.message);
