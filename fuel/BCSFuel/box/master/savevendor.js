/*
	...
	Description : CombineMaster Save Service for vendor
...
var saveVendorInterface = require("savevendorinterface.js");
...
try {
  ...
  var onLoadRes = null;
  if (saveVendorInterface && saveVendorInterface.onLoad)
    onLoadRes = saveVendorInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (saveVendorInterface && saveVendorInterface.onError)
          respObj = saveVendorInterface.onError(respObj);
    ...
    if (saveVendorInterface && saveVendorInterface.beforeData)
      beforeDataRes = saveVendorInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params && params.data) {
        ...
        if (saveVendorInterface && saveVendorInterface.beforeExecute)
          queryParams = saveVendorInterface.beforeExecute(queryParams, "query");
        ...
        if (masterRespObj) {
          ...
          if (masterObj) { //master already has data, new data should be appended
            ...
            if (saveVendorInterface && saveVendorInterface.beforeExecute)
              masterDataStr = saveVendorInterface.beforeExecute(masterDataStr, "update");
            ...
            if (saveVendorInterface && saveVendorInterface.beforeExecute)
              masterDataStr = saveVendorInterface.beforeExecute(masterDataStr, "insert");
    ...
    switch (respObj.code) {
      case 200:
        if (saveVendorInterface && saveVendorInterface.afterData)
          respObj.data = saveVendorInterface.afterData(respObj.data);
      ...
      case 400:
        ...
        if (saveVendorInterface && saveVendorInterface.onError)
          respObj = saveVendorInterface.onError(respObj);
        console.error("Unable to save the 'vendor' - " + paramStr);
        break;
      case 403:
        respObj.msg = Strings.ACCESS_DENIED;
        if (saveVendorInterface && saveVendorInterface.onError)
          respObj = saveVendorInterface.onError(respObj);
        console.error("User access denied when saving the 'vendor' - " + paramStr);
        break;
      case 500:
        respObj.msg = Strings.SERVER_ERROR;
        if (saveVendorInterface && saveVendorInterface.onError)
          respObj = saveVendorInterface.onError(respObj);
        console.error("Unexpected server error when saving the 'vendor' - " + paramStr);
        break;
    }
  ...
  if (saveVendorInterface && saveVendorInterface.onError)
    errorRespObj = saveVendorInterface.onError(errorRespObj);
  console.error("Unexpected server error when saving the vendor - " + paramStr + "-" + errorRespObj.message);
