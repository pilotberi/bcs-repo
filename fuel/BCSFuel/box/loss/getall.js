/*
	...
	Description : Get All Service for loss
...
var getAllLossDataInterface = require("getallinterface.js");
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.READ_BCSFUEL_LOSS_GETALLLOSSDATA_SUCCESS
  };
  ...
  var onLoadRes = null;
  if (getAllLossDataInterface)
    onLoadRes = getAllLossDataInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (getAllLossDataInterface)
          respObj = getAllLossDataInterface.onError(respObj);
    ...
    if (getAllLossDataInterface)
      beforeDataRes = getAllLossDataInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (getAllLossDataInterface) {
        params = getAllLossDataInterface.beforeExecute(params);
      }

      var countRespObj = null;

      var getAllLossDataQueryParams = {};


      //Setting offset and limit after count query
      if (params.offset)
        getAllLossDataQueryParams.offset = params.offset;

      if (params.limit)
        getAllLossDataQueryParams.limit = params.limit;

      if (params.offset || params.limit)
        countRespObj = dsclientUtil.execute(getAllLossDataQueryParams, "BCSFuel", "bcsfuel_loss", "count");

      //Sending request to ds client
      respObj = dsclientUtil.execute(params, "BCSFuel", "bcsfuel_loss", "getAll");
    ...
    switch (respObj.code) {
      case 200:
        if (getAllLossDataInterface)
          respObj.data = getAllLossDataInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_BCSFUEL_LOSSES_GETALLLOSSDATA_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_BCSFUEL_LOSSES_GETALLLOSSDATA_ERROR;
        if (getAllLossDataInterface)
          respObj = getAllLossDataInterface.onError(respObj);
  ...
  if (getAllLossDataInterface)
    errorRespObj = getAllLossDataInterface.onError(errorRespObj);
