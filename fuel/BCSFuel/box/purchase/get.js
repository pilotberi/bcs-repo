/*
	...
	Description : Get Service for purchase
...
var getPurchaseDataInterface = require("getinterface.js");
...
try {
  ...
  var respObj = {
    code: 200,
    msg: Strings.READ_BCSFUEL_PURCHASE_GETPURCHASEDATA_SUCCESS
  };
  ...
  var onLoadRes = null;
  if (getPurchaseDataInterface)
    onLoadRes = getPurchaseDataInterface.onLoad(params);
  ...
  if (onLoadRes && onLoadRes.override === true) {
    if (onLoadRes.resp) {
      if (onLoadRes.resp.code) {
        ...
        if (getPurchaseDataInterface)
          respObj = getPurchaseDataInterface.onError(respObj);
    ...
    if (getPurchaseDataInterface)
      beforeDataRes = getPurchaseDataInterface.beforeData(params);
    ...
    if (beforeDataRes && beforeDataRes.override === true) {
      ...
      if (params && params.id) {
        ...
        if (getPurchaseDataInterface) {
          getParams = getPurchaseDataInterface.beforeExecute(getParams);
        }
        //Sending request to ds client
        respObj = dsclientUtil.execute(getParams, "BCSFuel", "bcsfuel_purchase", "get");
      } else {
        respObj.code = 400;
    ...
    switch (respObj.code) {
      case 200:
        if (getPurchaseDataInterface)
          respObj.data = getPurchaseDataInterface.afterData(respObj.data);
        respObj.msg = Strings.READ_BCSFUEL_PURCHASE_GETPURCHASEDATA_SUCCESS;
        break;
      case 400:
        respObj.msg = Strings.READ_BCSFUEL_PURCHASE_GETPURCHASEDATA_ERROR;
        if (getPurchaseDataInterface)
          respObj = getPurchaseDataInterface.onError(respObj);
  ...
  if (getPurchaseDataInterface)
    errorRespObj = getPurchaseDataInterface.onError(errorRespObj);
