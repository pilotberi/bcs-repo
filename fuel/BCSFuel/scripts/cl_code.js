/*
	...
	Version : 2024
	Generated On : Tue Apr 07 2026 09:55:35 GMT+0530 (India Standard Time)
...
function onSaveCodeBtnClick(e) {
  ...
  for (var row = 0; row < sheetData.length; row++) {
    ...
    if (rowData[1] && rowData[1].length > 0) {
      ...
      data.code = rowData[1];
      data.unit = rowData[2];
      data.description = rowData[3];
      data.gst_applicable = rowData[4];
      data.gst_rate = rowData[5];
  ...
  var checkStatus = validate(codeArr);
...
function validate(dataArr) {
  ...
  dataArr.forEach(function(codeObj) {
    var errorStr = "";
    var rowNum = codeObj._rowNum;

    function addError(error) {
      errorStr += error + "<br>";
    }

    var rowTitle = codeObj.name || codeObj.undefined || codeObj.id || "-";

    if (CodeInterface && CodeInterface.getRowTitle) {
      var customTitle = CodeInterface.getRowTitle(codeObj, dataSources);
    ...
    if (codeObj.code == null || codeObj.code === '') {
      addError(Strings.CODE_INVALID_CODE);
    }

    // C. External Interface Validation
    if (CodeInterface && CodeInterface.addValidation) {
      var extErr = CodeInterface.addValidation(codeObj, dataSources);
  ...
  if (validateError.length > 0) {
    ...
    var promptTitle = (typeof Strings !== 'undefined' && Strings.VALIDATE_CODE_PROMPT) ? Strings.VALIDATE_CODE_PROMPT : "Validation Errors";
    var objTitle = (typeof Strings !== 'undefined' && Strings.CODE_TITLE) ? Strings.CODE_TITLE : "Code";
