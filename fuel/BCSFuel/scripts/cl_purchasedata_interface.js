...
var PurchaseInterface = {
	...
	onAfterLoad: function(dataSources) {
		...
		clCommon.disable(purchase_date_monthSelect);
		clCommon.disable(purchase_date_yearSelect);
		...
		if (!rowData || !rowData._sid) {
			if(fuelCodeFilter.value !=="all"){
				if(dataSources.getCode){
					for(var idx = 0; idx < dataSources.getCode.length; idx++){
						if(fuelCodeFilter.value == dataSources.getCode[idx].id){
							purchaseFuel_code_id.value = dataSources.getCode[idx].code;
							purchaseFuel_code_id.setAttribute("fuel_code_id", dataSources.getCode[idx].id);
							break;
						}
					}
				}
				...
				if(filter_monthSelect && filter_monthSelect.value){
					purchase_date_monthSelect.value = filter_monthSelect.value;
				}

				if(filter_yearSelect && filter_yearSelect.value){
					purchase_date_yearSelect.value = filter_yearSelect.value;
				}
			}
		}
