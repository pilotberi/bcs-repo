/*
	...
	Version : 2024
	Generated On : Tue Apr 07 2026 09:55:48 GMT+0530 (India Standard Time)
	Description : Combine Master Ui for unit
...
function onApplicationReady() {
  ...
  setMenu(document.getElementById('sideMenu'), 'bcsfuel_unit');
...
function registerEventListeners() {
  saveUnitBtn = document.getElementById('saveUnitBtn');
  saveUnitBtn.addEventListener('click', onSaveUnitBtnClick);
}
...
function onUser(resp) {
  ...
  clCommon.initLanguage(userObj.realm_lang, function() {
    ...
    if (typeof UnitInterface === 'undefined')
      UnitInterface = null;

    if (UnitInterface && UnitInterface.onLoad)
      UnitInterface.onLoad();
    initControls();
...
function loadUnitData() {
  ...
  var beforeLoad = true;
  if (UnitInterface && UnitInterface.onBeforeLoad)
    beforeLoad = UnitInterface.onBeforeLoad();
  if (beforeLoad == false) {
    return;
  }
  ...
  Service.getUnit(function(resultObj) {
    ...
    if (resultObj.code == 200) { //On success response
      ...
      var dataArr = resultObj.data;
      if (UnitInterface && UnitInterface.onAfterLoad) {
        ...
        dataArr = UnitInterface.onAfterLoad(isSuccessful, resultObj.data);
      }
      ...
      s_error(Strings.ERROR_TITLE, Strings.ERROR_LOAD_UNIT, dlgOptions);
    }
  });
...
function onLoadSuccessResponse(dataArr) {
  if (dataArr.length > 0) {
    for (var i = 0; i < dataArr.length; i++) {
      ...
      editorSheet.setValue('B' + rowId, dataObj.unit, true);
...
function onSaveUnitBtnClick(e) {
  ...
  var checkStatus = validate(unitArr);
  ...
  var unitObj = true;
  if (UnitInterface && UnitInterface.onBeforeSave)
    unitObj = UnitInterface.onBeforeSave(masterObj);
  if (unitObj == false) {
    return;
  }
  ...
  Service.saveUnit(masterObj, function(resultObj) {
    clCommon.hideLoader("listProgress");
    if (resultObj.code == 200) {
      var isSuccessful = true;

      var unitObj = true;
      if (UnitInterface && UnitInterface.onAfterSave)
        unitObj = UnitInterface.onAfterSave(isSuccessful, resultObj.data);
      ...
      if (isSuccessful && resultObj.data.master_data) {
        try {
          ...
          s_error(Strings.UNIT_ERROR_TITLE, Strings.SAVE_UNIT_ERROR, dlgOptions);
        }
      }
...
function validate(dataArr) {
  ...
  dataArr.forEach(function(unitObj) {
    var errorStr = "";
    var rowNum = unitObj._rowNum;

    function addError(error) {
      errorStr += error + "<br>";
    }

    var rowTitle = unitObj.name || unitObj.undefined || unitObj.id || "-";

    if (UnitInterface && UnitInterface.getRowTitle) {
      var customTitle = UnitInterface.getRowTitle(unitObj, dataSources);
    ...
    if (unitObj.unit == null || unitObj.unit === '') {
      addError(Strings.UNIT_INVALID_UNIT);
    }

    // C. External Interface Validation
    if (UnitInterface && UnitInterface.addValidation) {
      var extErr = UnitInterface.addValidation(unitObj, dataSources);
  ...
  if (validateError.length > 0) {
    ...
    var promptTitle = (typeof Strings !== 'undefined' && Strings.VALIDATE_UNIT_PROMPT) ? Strings.VALIDATE_UNIT_PROMPT : "Validation Errors";
    var objTitle = (typeof Strings !== 'undefined' && Strings.UNIT_TITLE) ? Strings.UNIT_TITLE : "Unit";
...
function onSheetChangedListener(instance, cell, col, row, value) {
  ...
  var returnData = true;
  if (UnitInterface && UnitInterface.onSheetChanged)
    returnData = UnitInterface.onSheetChanged(instance, cell, col, row, value);
  if (returnData == false) {
    return;
  }
