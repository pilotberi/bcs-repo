/*
	...
	Version : 2024
	Generated On : Tue Apr 07 2026 09:55:41 GMT+0530 (India Standard Time)
	Description : Combine Master Ui for yard
...
function onApplicationReady() {
  ...
  setMenu(document.getElementById('sideMenu'), 'bcsfuel_yard');
...
function initSheet() {
  ...
  editorSheet = jspreadsheet(sheetEl, {
    ...
    columns: [{
      ...
      {
        type: 'text',
        title: Strings.COL_LABEL_FUEL_YARD_CODE,
        width: 180,
        align: 'left',
        wordWrap: true,
        readOnly: false
...
function registerEventListeners() {
  saveYardBtn = document.getElementById('saveYardBtn');
  saveYardBtn.addEventListener('click', onSaveYardBtnClick);
}
...
function onUser(resp) {
  ...
  clCommon.initLanguage(userObj.realm_lang, function() {
    ...
    if (typeof YardInterface === 'undefined')
      YardInterface = null;

    if (YardInterface && YardInterface.onLoad)
      YardInterface.onLoad();
    initControls();
...
function loadYardData() {
  ...
  var beforeLoad = true;
  if (YardInterface && YardInterface.onBeforeLoad)
    beforeLoad = YardInterface.onBeforeLoad();
  if (beforeLoad == false) {
    return;
  }

  Service.getYard(function(resultObj) {
    ...
    if (resultObj.code == 200) { //On success response
      ...
      var dataArr = resultObj.data;
      if (YardInterface && YardInterface.onAfterLoad) {
        ...
        dataArr = YardInterface.onAfterLoad(isSuccessful, resultObj.data);
      }
      ...
      s_error(Strings.ERROR_TITLE, Strings.ERROR_LOAD_YARD, dlgOptions);
    }
  });
...
function onLoadSuccessResponse(dataArr) {
  if (dataArr.length > 0) {
    for (var i = 0; i < dataArr.length; i++) {
      ...
      editorSheet.setValue('B' + rowId, dataObj.yard_code, true);
...
function onSaveYardBtnClick(e) {
  ...
  var checkStatus = validate(yardArr);
  ...
  var yardObj = true;
  if (YardInterface && YardInterface.onBeforeSave)
    yardObj = YardInterface.onBeforeSave(masterObj);
  if (yardObj == false) {
    return;
  }
  ...
  Service.saveYard(masterObj, function(resultObj) {
    clCommon.hideLoader("listProgress");
    if (resultObj.code == 200) {
      var isSuccessful = true;

      var yardObj = true;
      if (YardInterface && YardInterface.onAfterSave)
        yardObj = YardInterface.onAfterSave(isSuccessful, resultObj.data);
      ...
      if (isSuccessful && resultObj.data.master_data) {
        try {
          ...
          s_error(Strings.YARD_ERROR_TITLE, Strings.SAVE_YARD_ERROR, dlgOptions);
        }
      }
...
function validate(dataArr) {
  ...
  dataArr.forEach(function(yardObj) {
    var errorStr = "";
    var rowNum = yardObj._rowNum;

    function addError(error) {
      errorStr += error + "<br>";
    }

    var rowTitle = yardObj.name || yardObj.undefined || yardObj.id || "-";

    if (YardInterface && YardInterface.getRowTitle) {
      var customTitle = YardInterface.getRowTitle(yardObj, dataSources);
    ...
    if (yardObj.yard_code == null || yardObj.yard_code === '') {
      addError(Strings.YARD_INVALID_YARD_CODE);
    }

    // C. External Interface Validation
    if (YardInterface && YardInterface.addValidation) {
      var extErr = YardInterface.addValidation(yardObj, dataSources);
  ...
  if (validateError.length > 0) {
    ...
    var promptTitle = (typeof Strings !== 'undefined' && Strings.VALIDATE_YARD_PROMPT) ? Strings.VALIDATE_YARD_PROMPT : "Validation Errors";
    var objTitle = (typeof Strings !== 'undefined' && Strings.YARD_TITLE) ? Strings.YARD_TITLE : "Yard";
...
function onSheetChangedListener(instance, cell, col, row, value) {
  ...
  var returnData = true;
  if (YardInterface && YardInterface.onSheetChanged)
    returnData = YardInterface.onSheetChanged(instance, cell, col, row, value);
  if (returnData == false) {
    return;
  }
